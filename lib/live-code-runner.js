...
import view from './live-code-runner-view';
...
export default AtomLiveCodeRunner = {
  ...
  realActivate(state) {
    this.view = new view(state.viewState, this);
    this.resultPanel = atom.workspace.addBottomPanel({ item: this.view.getElement(), visible: false });
  ...
  ensureClient() {
    if (this.clientConfig === null) {
      ...
      let pkgVersion = '5.1.0';  // TODO: read from package.json
  ...
  createKernel(kernelType) {
    ...
    return this.client.createKernel(kernelType)
      .then(response => {
        ...
        this.view.setKernelIndicator(kernelType);
        return true;
      }).catch(err => {
  ...
  sendCode() {
    ...
    if (this.waiting_input === true) {
      ...
      this.view.addLogMessage('[LOG] Running...');
      let editor = atom.workspace.getActiveTextEditor();
    ...
    return this.client.runCode(this.code, this.kernelId, this.runId, mode)
      .then(response => {
        ...
        if (response.result.exceptions && (response.result.exceptions.length > 0)) {
          ...
          this.view.setErrorMessage(errBuffer);
        }
        this.view.appendContent(buffer);
        if (this.waiting_input === true) {
          this.view.addInputWidget();
        }
        return true;
      }).catch(err => {
        this.view.setErrorMessage(`live-code-runner: ${err.message}`);
        switch (err.type) {
