...
import SornaAPILib from './sorna-api-lib-v2';
          ...
          response.json().then( json => {
            ...
            if (json.result.console) {
              for (let c of Array.from(json.result.console)) {
                if (c[0] == 'stdout') {
                  buffer = buffer + '<br /><pre>'+this.escapeHtml(c[1])+'</pre>';
                }
                if (c[0] == 'stderr') {
                  buffer = buffer + '<br /><pre class="live-code-runner-error-message">'+this.escapeHtml(c[1])+'</pre>';
                }
                if (c[0] == 'media') {
